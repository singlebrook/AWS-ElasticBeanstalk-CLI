    ...
    def write(self, pathfilename):
        with codecs.open(pathfilename, 'w', encoding=ServiceDefault.CHAR_CODEC) as output_file:
            for section in self._sections:
                output_file.write('[{0}]{1}'.format(section.strip(), os.linesep))
                for (option, value) in list(self._sections[section].items()):
                    if option == "__name__":
                        continue
                    output_file.write('{0}={1}{2}'.format(option.strip(), value.strip(), os.linesep))
    ...
    def write(self, pathfilename):
        with codecs.open(pathfilename, 'w', encoding=ServiceDefault.CHAR_CODEC) as output_file:
            for option in self.options(self._default_section):
                output_file.write('{0}={1}{2}'.format(option, self.get(option), os.linesep))
